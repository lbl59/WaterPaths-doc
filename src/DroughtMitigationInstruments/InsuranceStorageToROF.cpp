...
void InsuranceStorageToROF::priceInsurance(int week) {
    ...
    for (int r = 0; r < NUMBER_REALIZATIONS_ROF; ++r) {
        ...
        // Run short-term ROF simulations for current realization across all weeks in the year.
        for (int w = week - (int) WEEKS_IN_YEAR + 1; w <= week; ++w) {
            // One week continuity time-step.
            continuityStep(w, r);

            // Get utilities' approximate short-term ROF from storage-ROF-table.
            ...
            // Increase the price of the insurance if payout is triggered and reset drought mitigation policy's utility-variables.
    ...
    // Average prices across realizations and updates each utility's insurance price.
...
vector<double> InsuranceStorageToROF::calculateShortTermROFTable(int week,
    ...
    for (int u = 0; u < n_utilities; ++u) {
        ...
        // Calculate ratio of current and status-quo utility storage capacities (m)
        ...
        // Mean ROF between the two tiers of the ROF table where current storage is located.
