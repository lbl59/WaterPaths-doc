...
void Graph::addEdgeToEdgesList(int u, int v) {
    // Append vertex `v` to the adjacency list of vertex `u`.
    adj[u].push_back(v);
    // Append vertex `v` to the downstream sources list for vertex `u`.
...
void Graph::addEdge(int u, int v) {
    ...
    // Create and initialize a vector to represent the added edge.
    vector<double> pipe_connectivity((unsigned long) V, 0);
    pipe_connectivity[u] = -1;
    pipe_connectivity[v] = 1;

    // Append the vector to the `continuity_matrix_transpose`.
...
vector<int> Graph::findUpstreamSources(int id) const {
    ...
    for (int i = 0; i < V; ++i) {
        // For each vertex, check if `id` is present in its adjacency list
        // If found, add the vertex index to `upstream_sources`.
...
const vector<vector<double>> Graph::getContinuityMatrix() const {
    ...
    // Expand the matrix to include identity terms by adding zero rows and columns.
    for (int i = 0; i < (int) continuity_matrix.size(); ++i) {
        for (int ii = 0; ii < (int) continuity_matrix.size(); ++ii) {
            continuity_matrix[i].push_back(0);
        }
        // Set diagonal elements in the new section of the matrix to `-1`.
