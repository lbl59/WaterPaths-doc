...
#ifdef TRACE_SOLVER
    std::cout << "Partial step has been taken " << t << std::endl;
    print_vector("allocations_aux", allocations_aux);
#endif
    // Drop constraint l
    iai[l] = l;
...
void print_matrix(char *name, const Matrix<double> &A, int n, int m) {
    std::ostringstream s;
    std::string t;
    // If `n` or `m` are set to -1, determine the actual number of rows and columns from the matrix.
    if (n == -1)
        n = A.nrows();
    if (m == -1)
        m = A.ncols();

    // Format the matrix as a string, appending its name as a label.
    s << name << ": " << std::endl;
    for (int i = 0; i < n; i++) {
        s << " ";
        for (int j = 0; j < m; j++)
    ...
    t = s.str();
    // Remove the trailing comma and newline from the last row for cleaner formatting.
    t = t.substr(0, t.size() - 3);
    // Output the formatted matrix string to the console.
    std::cout << t << std::endl;
}
...
void print_vector(char *name, const Vector<double> &v, int n) {
    std::ostringstream s;
    std::string t;
    // If `n` is set to -1, determine the actual number of elements from the vector.
    if (n == -1)
        n = v.size();

    // Format the vector elements as a string, appending its name as a label.
    s << name << ": " << std::endl << " ";
    for (int i = 0; i < n; i++) {
        s << v[i] << ", ";
    }
    t = s.str();
    // Remove the trailing comma and space from the last element for cleaner formatting./
    t = t.substr(0, t.size() - 2);

    // Output the formatted vector string to the console.
    std::cout << t << std::endl;
}
