...
vector<vector<double>> Utils::parse2DCsvFile(string file_name, unsigned long max_lines,
    ...
    // Determine the maximum line to read.
    int ml = (int) (rows_to_read.empty() ? max_lines : *max_element(
                            rows_to_read.begin(), rows_to_read.end())) + 1;
    // Parse the line into double values and store them in a 2D vector.
...
vector<MinEnvFlowControl *> Utils::copyMinEnvFlowControlVector(
    ...
    for (MinEnvFlowControl *mef : min_env_flow_controls_original) {
        // Identify the type of each control.
        // Create a copy of the control and add it to the new vector.
...
vector<WaterSource *> Utils::copyWaterSourceVector(
    ...
    // Identify the type of each water source.
    // Create a copy of the water source and add it to the new vector.
...
vector<Utility *> Utils::copyUtilityVector(vector<Utility *> utility_original,
    ...
    // Create a copy of the utility and add it to the new vector.
...
Utils::copyDroughtMitigationPolicyVector(
    ...
    // Identify the type of each policy.
    // Create a copy of the policy and add it to the new vector.
...
vector<Bond *> Utils::copyBonds(vector<Bond *> bonds_original) {
    ...
    // Identify the type of each bond and create a copy of it.
    // Add the bond to the new vector
