...
void VariableJointWTP::resetAllocations(const vector<double> *demand_deltas) {
    // Take estimate of future expected next-year demand compared to current demand 
    // Adjusts the treatment allocation based on `demand_deltas` for the JointWTP for the rest of the year
    for (int u : *utilities_with_allocations) {
        allocated_treatment_capacities[u] += demand_deltas->at(u);

        // Ensure that no treatment allocation goes below zero.
    ...
    // If the total allocated capacity exceeds the available treatment capacity, 
    // adjusts the fractions and capacities accordingly.
