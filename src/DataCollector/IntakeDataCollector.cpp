...
string IntakeDataCollector::printTabularString(int week) {
    ...
    outStream
            ...
            << demands[week] // demands for the week
            << setw(COLUMN_WIDTH) << setprecision(COLUMN_PRECISION)
            << total_upstream_sources_inflows[week]  // total upsteam inflows for the intake
            << setw(COLUMN_WIDTH) << setprecision(COLUMN_PRECISION)
            << wastewater_inflows[week]  // total wastewater inflows for the intake
            << setw(COLUMN_WIDTH) << setprecision(COLUMN_PRECISION)
            << total_catchments_inflow[week]   // total catchment inflows for the intake
            << setw(COLUMN_WIDTH) << setprecision(COLUMN_PRECISION)
            << outflows[week];  // total outflows for the intake
